...
app.config["SQLALCHEMY_DATABASE_URI"] = (
    database_uri or "sqlite:///confeitando_com_artes.db"
)
...
def index():
    ...
    ingredientes_estoque_grafico = (
        Ingrediente.query.order_by(Ingrediente.quantidade_estoque.desc()).limit(5).all()
    )
    ...
    condicao_critica = db.or_(
        db.and_(
            ...
            Ingrediente.quantidade_estoque <= 100,
        ),
        db.and_(
            Ingrediente.unidade_medida == "unid", Ingrediente.quantidade_estoque <= 5
        ),
    )
    condicao_alerta = db.or_(
        db.and_(
            Ingrediente.unidade_medida.in_(["g", "ml"]),
            Ingrediente.quantidade_estoque > 100,
            Ingrediente.quantidade_estoque <= 500,
        ),
        db.and_(
            Ingrediente.unidade_medida == "unid",
            Ingrediente.quantidade_estoque > 5,
            Ingrediente.quantidade_estoque <= 15,
        ),
    )

    # 3. Cria a query base para os alertas
    query_alertas = Ingrediente.query.filter(
        db.or_(condicao_critica, condicao_alerta)
    ).order_by(Ingrediente.quantidade_estoque.asc())

    # 4. Pagina os resultados, mostrando 5 por página
    paginacao_alertas = query_alertas.paginate(
        page=page_alertas, per_page=5, error_out=False
    )
...
def gerenciar_ingredientes():
    if request.method == "POST":
        ...
        if Ingrediente.query.filter(
            ...
            flash(
                f'Erro: O ingrediente "{nome_original}" já está cadastrado.', "danger"
            )
...
def adicionar_estoque(ingrediente_id):
    ...
    nova_compra = Compra(
        preco_total_lote=preco_lote,
        unidades_compradas=unidades_lote,
        tamanho_unidade=tamanho_unidade_lote,
        ingrediente_id=ingrediente_id,
    )
    db.session.add(nova_compra)
    estoque_antigo_total = ingrediente.quantidade_estoque
    valor_total_estoque_antigo = (
        estoque_antigo_total * ingrediente.custo_medio_por_unidade_base
    )
    ...
    novo_custo_medio = (
        novo_valor_total / novo_estoque_total if novo_estoque_total > 0 else 0
    )
    ingrediente.quantidade_estoque = novo_estoque_total
    ingrediente.custo_medio_por_unidade_base = novo_custo_medio
    db.session.commit()
    flash(
        f'Estoque de "{ingrediente.nome}" atualizado! Novo custo médio: R$ {novo_custo_medio:.4f}',
        "success",
    )
    return redirect(url_for("gerenciar_ingredientes"))
...
def editar_ingrediente(ingrediente_id):
    ...
    if request.method == "POST":
        ...
        outro_ingrediente = Ingrediente.query.filter(
            Ingrediente.id != ingrediente_id,
            db.func.lower(Ingrediente.nome) == db.func.lower(novo_nome),
        ).first()
...
def historico_compras(ingrediente_id):
    ingrediente = Ingrediente.query.get_or_404(ingrediente_id)
    historico = (
        Compra.query.filter_by(ingrediente_id=ingrediente.id)
        .order_by(Compra.data_compra.desc())
        .all()
    )
    return render_template(
        "historico_compras.html", ingrediente=ingrediente, historico=historico
    )
...
def gerenciar_formas():
    if request.method == "POST":
        ...
        if Forma.query.filter(
            db.func.lower(Forma.descricao) == db.func.lower(descricao)
        ).first():
    ...
    paginacao = query.order_by(Forma.descricao).paginate(
        page=page, per_page=5, error_out=False
    )
    formas_da_pagina = paginacao.items

    return render_template(
        "formas.html", formas=formas_da_pagina, paginacao=paginacao, q=q
    )
...
def gerenciar_fichas_tecnicas():
    if request.method == "POST":
        ...
        ficha_existente = FichaTecnica.query.filter(
            FichaTecnica.nome_normalizado == nome_normalizado
        ).first()
        if ficha_existente:
            flash(
                f'Uma ficha técnica com o nome "{nome_original}" já existe.', "danger"
            )
            return redirect(url_for("gerenciar_fichas_tecnicas"))
        ...
        return redirect(
            url_for("detalhe_ficha_tecnica", ficha_tecnica_id=nova_ficha.id)
        )
...
def detalhe_ficha_tecnica(ficha_tecnica_id):
    ...
    if request.method == "POST":
        ...
        if "atualizar_ficha" in request.form:
            ...
            ficha.tempo_producao_horas = int(
                request.form.get("tempo_producao_horas", 0) or 0
            )
            ficha.tempo_producao_minutos = int(
                request.form.get("tempo_producao_minutos", 0) or 0
            )

            ficha.incluir_custo_mao_de_obra = (
                "incluir_custo_mao_de_obra" in request.form
            )

            ids_formas_selecionadas = request.form.getlist("formas_selecionadas")
            ficha.formas = Forma.query.filter(
                Forma.id.in_(ids_formas_selecionadas)
            ).all()
    ...
    paginacao_ingredientes = FichaTecnicaIngrediente.query.filter_by(
        fichatecnica_id=ficha.id
    ).paginate(page=page_ing, per_page=5, error_out=False)
...
def produzir_ficha_tecnica(ficha_tecnica_id):
    ...
    try:
        ...
        flash(
            "A quantidade a produzir deve ser um número inteiro maior que zero.",
            "warning",
        )
        return redirect(
            url_for("detalhe_ficha_tecnica", ficha_tecnica_id=ficha_tecnica_id)
        )
    ingredientes_insuficientes = [
        item.ingrediente.nome
        for item in ficha.ingredientes
        if item.ingrediente.quantidade_estoque
        < (item.quantidade_usada * quantidade_a_produzir)
    ]
    if ingredientes_insuficientes:
        flash(
            f'Não foi possível produzir. Estoque insuficiente para: {", ".join(ingredientes_insuficientes)}.',
            "danger",
        )
        return redirect(
            url_for("detalhe_ficha_tecnica", ficha_tecnica_id=ficha_tecnica_id)
    ...
    for item in ficha.ingredientes:
        item.ingrediente.quantidade_estoque -= (
            item.quantidade_usada * quantidade_a_produzir
        )
    db.session.commit()
    flash(
        f'{quantidade_a_produzir} unidade(s) de "{ficha.nome}" produzida(s) com sucesso! Estoque atualizado.',
        "success",
    )
...
def gerenciar_vendas():
    if request.method == "POST":
        ...
        if not fichatecnica_id or not preco_venda_str:
            flash(
                "Por favor, selecione uma ficha e preencha o preço de venda.", "warning"
            )
        ...
        try:
            ...
            flash(
                "Por favor, insira valores numéricos válidos para quantidade e preço.",
                "danger",
            )
            return redirect(url_for("gerenciar_vendas"))
...
def gerar_pdf_orcamento():
    ...
    if data_pedido_str:
        dados_orcamento["data_pedido"] = datetime.strptime(
            data_pedido_str, "%Y-%m-%d"
        ).strftime("%d/%m/%Y")

    # Formata a data do evento/entrega
    data_evento_str = dados_orcamento.get("data_evento")
    if data_evento_str:
        dados_orcamento["data_evento"] = datetime.strptime(
            data_evento_str, "%Y-%m-%d"
        ).strftime("%d/%m/%Y")
    # --- FIM DA CORREÇÃO ---
    ...
    return send_file(
        result, mimetype="application/pdf", as_attachment=True, download_name=filename
    )
...
def editar_forma(id):
    ...
    outra_forma = Forma.query.filter(
        Forma.id != id, db.func.lower(Forma.descricao) == db.func.lower(nova_descricao)
    ).first()
    if outra_forma:
...
def setup_inicial():
    try:
        with app.app_context():
            # O comando 'flask db upgrade' no build já deve ter criado as tabelas.
            # Esta rota agora foca em criar os usuários.
